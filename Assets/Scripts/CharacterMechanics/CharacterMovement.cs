...
public class CharacterMovement : MonoBehaviour
{
    ...
    // feel free to remove the nonserialized tags from these if you want to view them in the editor
    // they're only not shown to not overwhelm the vast majority of students that don't open the code and tinker
    [NonSerialized]
    ...
    Vector3 ApplyMovementVelocity(Vector3 additionalImpulse)
    {
        ...
        Vector3 combinedVelocity = verticalVelocity + moveVelocity - dx * GravityUpDirection;
    ...
    void SnapCharacterToGround()
    {
        ...
        {
            transform.position = pointHit.point + (Controller.height/2 + Controller.skinWidth) * GravityUpDirection;
        } else if (didCapsuleHit && !isCapsuleOverGeometry)
        {
    ...
    void UpdateTrackedPlatform()
    {
        ...
        PlatformTrackingGhost.SetParent(onPlatform && DoPlatformTracking ? currentPlatform.transform : transform);
    ...
    private void LateUpdate()
    {
        ...
        Vector3 platformMovement = DoPlatformTracking ? PlatformTrackingGhost.position - transform.position : Vector3.zero;
